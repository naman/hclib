 ...
 *
 * NOTE: packing a unique_ptr into a promise_t is not currently supported,
 * so we can't use std::make_unique as one would usually do.
 */
int main(int argc, char ** argv) {
    setbuf(stdout,nullptr);
    int n = 5;
    hclib::promise_t<std::unique_ptr<int>*> **promise_list = new hclib::promise_t<std::unique_ptr<int>*> *[n+1];
    hclib::launch([=]() {
        hclib::finish([=]() {
            int index = 0;
            // Create asyncs
            for (index = 0 ; index <= n; index++) {
                promise_list[index] = new hclib::promise_t<std::unique_ptr<int>*>();
            ...
            for(index=n-1; index>=1; index--) {
                ...
                hclib::async_await([=]() {
                    ...
                    promise_list[index]->put(
                            new std::unique_ptr<int>(new int(index)));
                }, promise_list[index-1]->get_future());
            }

            printf("Putting in promise 0\n");
            promise_list[0]->put(new std::unique_ptr<int>(new int(0)));
        });
